...
const TodoCard = ({ id, task_title, notes, date }) => {
  ...
  return (
    <div className="todo-card">
      ...
      <div>
        {isEdit ? <input className="input" onChange={(e) => setCurrentTaskTitle(e.target.value)} value={currentTaskTitle} /> : <div className="font-bold text-xl capitalize">{task_title}</div>}
        ...
        {isEdit ? <textarea className="input" onChange={(e) => setCurrentNotes(e.target.value)} value={currentNotes} /> : <div className="text-[12px] capitalize">{notes}</div>}
